...
import com.muibsols.chefmeplease.Listeners.SauceClickListener
...
class SauceAdapter (val context: Context?,val sauceClickListener: SauceClickListener,val limit:Int) : RecyclerView.Adapter<SauceAdapter.ViewHolder>() {
    var sauces : List<Sauce> = ArrayList<Sauce>()
    var selected = 0
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val sauce = sauces[position]
        holder.name.text = sauce.name+" $"+sauce.price
        ...
        holder.itemView.setOnClickListener {
            if (sauce.selected) {
                ...
                sauceClickListener.onClickRemoveSauce(sauce)
                selected--
                ...
                if (limit == selected) {
                    ...
                    sauceClickListener.onClickAddSauce(sauce)
                    selected++
                }
